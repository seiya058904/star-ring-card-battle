(function (global) {
  const files = {
    ...
    "resonance-ready": "bong_001.ogg", "resonance-activate": "confirmation_003.ogg", "advanced-skill": "bong_001.ogg", "ultimate-impact": "confirmation_004.ogg", "battle-start": "open_001.ogg", "turn-end": "switch_002.ogg", "turn-start": "open_002.ogg", victory: "confirmation_004.ogg", defeat: "back_002.ogg"
  };
  const cache = new Map();
  ...
  const manager = {
    ...
    playCard(card) { const event = card.effectType === "heal" ? "heal" : ["shield", "defense"].includes(card.effectType) ? "shield" : card.effectType === "summon" ? "summon" : card.effectType === "burn" ? "burn" : card.effectType === "curse" ? "curse" : ["freeze", "control"].includes(card.effectType) ? "freeze" : card.effectType === "buff" ? "buff" : card.effectType === "debuff" ? "debuff" : card.skillTier === "special" ? "ultimate-impact" : card.skillTier === "advanced" ? "advanced-skill" : ["attack", "pierce", "execute", "lifesteal"].includes(card.effectType) ? "attack-hit" : "card-play"; this.play(event); },
