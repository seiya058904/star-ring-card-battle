(function (global) {
  ...
  function aiCardScore(card, context = {}) { const value = effect => typeof global.resolveCardEffectAmount === "function" ? global.resolveCardEffectAmount(effect, context.actor || {}, card) : (Number(effect.ratio || effect.amount || 0) * 1e6) * (Number(card.effectMultiplier) || 1); const actor = context.actor || {}; const missingHp = Math.max(0, (Number(actor.maxHp) || 0) - (Number(actor.hp) || 0)); const selfStatuses = actor.statuses || []; const foeStatuses = context.target?.statuses || []; const dmgVal = effect => { const raw = value(effect); const slay = effect.slayRace && context.target?.race === effect.slayRace ? (effect.slayMultiplier || 2) : 1; return raw * slay; }; let score = (card.effects || []).reduce((total, effect) => { if (effect.type === "damage") { const dv = dmgVal(effect); return total + dv * (context.target?.hp <= dv ? 2 : 1); } if (effect.type === "heal") { const gain = missingHp > 0 ? Math.min(value(effect), missingHp) : value(effect) * .05; return total + gain * (context.enemyLowHp ? 1.8 : .25); } if (effect.type === "shield") return total + value(effect) * (context.enemyLowHp ? 1.5 : .4); if (effect.type === "status") { const selfSide = ["增幅", "减伤", "灵巧防御", "连锁", "复生", "真实"].includes(effect.status); const already = (selfSide ? selfStatuses : foeStatuses).some(s => s.type === effect.status); return total + (already ? 200 : 1200) + value(effect); } if (effect.type === "draw") return total + (context.handSize < 3 ? 800 : 100); if (effect.type === "energy") return total + 400; if (effect.type === "cleanse") return total + 300; return total + 100; }, 0) / Math.max(1, Number(card.effectiveCost ?? card.cost ?? 1)); const style = context.style; if (style === "curse" && card.effects?.some(effect => ["诅咒", "燃烧"].includes(effect.status))) score *= 12; if (style === "curse" && context.playerHasCurse && card.effects?.some(effect => effect.type === "damage")) score *= 6; if (style === "control" && card.effects?.some(effect => ["冻结", "禁锢"].includes(effect.status))) score *= 8; if (style === "aggressive" && card.effects?.some(effect => effect.type === "damage")) score *= context.playerLowHp ? 8 : 5; if (style === "guardian" && card.effects?.some(effect => ["shield", "heal"].includes(effect.type))) score *= 8; if (style === "guardian" && card.skillTier === "advanced") score *= 2; if (style === "adaptive") { if (context.enemyLowHp && card.effects?.some(effect => ["heal", "shield"].includes(effect.type))) score *= 6; else if (context.playerLowHp && card.effects?.some(effect => effect.type === "damage")) score *= 6; else if (context.enemyLowHp) score *= 3; } return score; }
