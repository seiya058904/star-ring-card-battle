  ...
  const buildEffects = (tierName, cost, element, variant, name = "") => {
    ...
    if (tierName === "special") {
      ...
      const SPECIAL_CARD_RULES = {
        ...
        // 恢复：对恶魔造成 2 倍伤害。
        "斩魔剑": () => [slayer(1.15, "恶魔")],
        // 星界放逐：基线 1360839 的效果为 damage.ratio = 0.1776 + 3 回合暗属性诅咒(burnRatio .01344)，
        // 实战直接伤害再由 fixed-game-rules 按显示名对 damage 追加 ×1.35（实测 774140452 → 1045089610）。
        // 这里把该 1.35 作为效果自身字段 damageMultiplier 写入固定卡数据（与"按显示名加成"
        // 不同：它随卡牌数据走，改名/换显示名都不会变），保留原 ratio 以维持基线相同的取整次序。
        "星界放逐": () => [effect("damage", { ratio: 0.1776, element, damageMultiplier: 1.35 }), statusDot("诅咒", .01344, 3)],
